...
function setCommandInSession(intent, session, callback) {
    ...
    const selectedCommand = intent.slots.Commands;
    let repromptText = '';
    ...
    if (selectedCommand) {
        const selectedCommand = selectedCommand.value;
        sessionAttributes = createCommandAttributes(selectedCommand);
        speechOutput = `I now know your favorite color is ${selectedCommand}. You can ask me ` +
...
function getCommandFromSession(intent, session, callback) {
    ...
    if (session.attributes) {
        command = session.attributes.selectedCommand;
    }

    if (command) {
        speechOutput = `Your favorite color is ${command}. Goodbye.`;
